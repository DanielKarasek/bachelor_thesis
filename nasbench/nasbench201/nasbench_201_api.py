...
# [2020.02.25] NASBENCH_201_data.pth : 6219 architectures are trained once, 1621 architectures are trained twice, 7785 architectures are trained three times. `LESS` only supports CIFAR10-VALID.
...
class NASBench201API(NASBenchMetaAPI):
  ...
  def __init__(self,
    ...
    dataset_dict = self._get_dataset_dict(dataset_path, dataset_dict)

    keys = ('meta_archs', 'arch2infos', 'evaluated_indexes')

    for key in keys:
      assert key in dataset_dict, 'Can not find key[{:}] in the dict'.format(key)
    ...
    self.meta_archs = dataset_dict['meta_archs']
    self._avaliable_hps = {'12', '200'}

    for xkey in sorted(list(dataset_dict['arch2infos'].keys())):
      all_info = dataset_dict['arch2infos'][xkey]
    ...
    self.evaluated_indexes = sorted(list(dataset_dict['evaluated_indexes']))
    self.archstr2index = {}
    for idx, arch in enumerate(self.meta_archs):
      assert arch not in self.archstr2index, (f'This [{idx}]-th arch {arch} ' 
                                              f'already in the dict ({self.archstr2index[arch]}).')
      self.archstr2index[arch] = idx

  def _get_dataset_dict(self, dataset_path: Optional[Text], dataset_dict: Optional[Dict], verbose: bool = True):
    """
    Init function to factor all dataset_dict magic out of __init__
    """
    logging.info("NASBench201API: Loading dict")
    if dataset_dict is None and dataset_path is None:
      dataset_path = os.path.join(os.environ['TORCH_HOME'], ALL_BENCHMARK_FILES[-1])
      logging.info('Try to use the default NAS-Bench-201 path from {:}.'.format(dataset_path))
    if dataset_path is not None:
      dataset_path = str(dataset_path)
      if verbose:
        logging.info('try to create the NAS-Bench-201 api from {:}'.format(dataset_path))
      assert os.path.isfile(dataset_path), 'invalid path : {:}'.format(dataset_path)
      self.filename = Path(dataset_path).name
      dataset_dict = torch.load(dataset_path, map_location='cpu')
    elif dataset_dict is not None:
      dataset_dict = dataset_dict
    assert isinstance(dataset_dict, dict), 'It should be a dict instead of {:}'.format(type(dataset_dict))
    logging.info("NASBench201API: Loaded dict")
    return dataset_dict
  ...
  def query_info_str_by_arch(self, arch, hp: Text = '12'):
    ...
    """
    logging.info('Call query_info_str_by_arch with arch={:} and hp={:}'.format(arch, hp))
    return self._query_info_str_by_arch(arch, hp, print_information)

  def get_more_info(self, index, dataset, iepoch=None, hp='12', is_random=False):
    """
    ...
    """
    logging.info(
      'Call the get_more_info function with index={:}, dataset={:}, iepoch={:}, hp={:}, and is_random={:}.'.format(
        index, dataset, iepoch, hp, is_random))
...
>>>>>>> 9065cc3cb3fc80a960d72274d2dc4fc463996d31
  ...
  def str2matrix(arch_str: Text,
                 search_space: List[Text] = ['none', 'skip_connect', 'nor_conv_1x1', 'nor_conv_3x3',
                                             'avg_pool_3x3']) -> Tuple[np.ndarray, List[Text]]:
    """
  ...
  @staticmethod
  def str2matrix_and_ops(arch_str: Text,
                         search_space: List[Text] = ['none', 'skip_connect', 'nor_conv_1x1', 'nor_conv_3x3',
                                                     'avg_pool_3x3']) -> Tuple[np.ndarray, List[Text]]:
    """
    This func shows how to convert the string-based architecture encoding to the encoding strategy in NAS-Bench-101.
    ...
    """
    edge_based_matrix, ops = NASBench201API.str2matrix(arch_str, search_space)
    node_based_matrix = np.array([[0, 1, 1, 1, 0, 0, 0, 0],  # input
                                  [0, 0, 0, 0, 1, 1, 0, 0],  # op1
                                  [0, 0, 0, 0, 0, 0, 1, 0],  # op2
                                  [0, 0, 0, 0, 0, 0, 0, 1],  # op3
                                  [0, 0, 0, 0, 0, 0, 1, 0],  # bop1
                                  [0, 0, 0, 0, 0, 0, 0, 1],  # bop2
...
>>>>>>> 9065cc3cb3fc80a960d72274d2dc4fc463996d31
    ...
    for idx, op in enumerate(ops):
      if op == 'none':
        node_based_matrix[idx, :] = 0
    return node_based_matrix, ops
